...
import { Form, TextField, Label, Button } from "@heroui/react"; // HeroUI v3 Imports
...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      console.log("ImgBB Full Response Log:", result);
      ...
      if (result.success) {
        ...
        console.log("MongoDB Ready Data Object:", finalUserData);
      } else {
        toast.error(`ImgBB Error: ${result.error?.message || "Upload Failed"}`);
      }
    } catch (error) {
  ...
  const handleGoogleSignIn = () => {
    console.log("Google Sign-In Clicked (Default Role: User)");
  };
    ...
    <div className="min-h-[calc(100vh-100px)] flex items-center justify-center bg-[#F5EBEB] px-4 py-12">
      <div className="w-full max-w-lg bg-[#E4D0D0]/50 backdrop-blur-md border border-[#E4D0D0] rounded-2xl p-6 md:p-8 shadow-sm">
        <div className="text-center mb-8 space-y-2">
          ...
          <p className="text-sm text-[#867070]/80">
        ...
        >
          {/* Full Name Field (With Demo Form Validation) */}
              ...
              {...register("name", { 
                ...
                validate: {
                  ...
                  minLengthThree: (value) =>
                    value.trim().length >= 3 || "Name must contain at least 3 characters",
                }
              })}
            />
            {errors.name && (
          ...
          {/* Submit Button (With Dynamic Loading State Text) */}
          <Button
            type="submit"
        ...
        <p className="text-center text-sm text-[#867070]/70 mt-6">
