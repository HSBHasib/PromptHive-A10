...
const PromptForm = ({ initialData = null, onSubmit, isSubmitting, imagePreview, setImagePreview, handleImageChange }) => {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  }, [initialData, reset, setImagePreview, imagePreview]);
  ...
  return (
    <form className="w-full space-y-6" onSubmit={handleSubmit(onSubmit)}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
          ...
          <div className="flex flex-col gap-1.5 relative" ref={toolRef}>
            ...
            {isToolOpen && (
              <div className="absolute top-18 left-0 w-full bg-white border border-[#86707020] rounded-xl shadow-lg z-50 overflow-hidden max-h-60 overflow-y-auto py-1">
                {aiTools.map((tool) => (
                  <div
                    key={tool.key}
                    onClick={() => {
      ...
      <div className="w-full pt-2">
        ...
        >
          {initialData ? "Update Prompt Details" : "Submit Prompt for Approval"}
        </Button>
      </div>
    </form>
  );
